...
# Read in hourly PM2.5 (FRM/FEM Mass) files, change
# to `tbl_df`, modify column names, save as .rdata
# file with bzip2 compression
for (i in 2008:2015){
  raw <- 
    read.csv(
      paste0("~/Downloads/hourly_88101_", i, ".csv"),
  ...
  pm25_frm_fem_hourly$param <- "pm25"
  pm25_frm_fem_hourly$unit_meas <- "ug_cm3"
  
  save(pm25_frm_fem_hourly,
       file = paste0("data/pm25_frm_fem_hourly_", i, ".rdata"),
       compress = "bzip2")
}
